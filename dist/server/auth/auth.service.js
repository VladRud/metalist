...
exports.setFacebookTokenCookie = setFacebookTokenCookie;
exports.setGoogleTokenCookie = setGoogleTokenCookie;
 ...
 */
function setFacebookTokenCookie(req, res) {
  if (!req.user) {
 ...
 */
function setGoogleTokenCookie(req, res) {
  if (!req.user) {
    return res.status(404).send('Похоже, вы не авторизованы. Попробуйте еще.');
  }
  var token = signToken(req.user._id, req.user.role);
